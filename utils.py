...
def parse_params(spec):
    """
    Parse strings like:
      "CAdamW,lr=5e-4,weight_decay=0.01,caution_eps=1e-3,local_scale=True,betas=(0.9,0.999)"
    → {"name":"CAdamW","lr":5e-4,"weight_decay":0.01,"caution_eps":1e-3,"local_scale":True,"betas":(0.9,0.999)}
    """
    import ast, re
    def _smart_split(s):
        parts, buf, depth = [], [], 0
    ...
    for p in parts:
        ...
        try:
            ...
            if re.fullmatch(fr'\s*{num_re}\s*,\s*{num_re}\s*', v):
                parsed = tuple(float(x) for x in v.split(','))
            else:
                parsed = v
        out[k] = parsed

    if 'optimizer' in out and 'name' not in out:
        out['name'] = out.pop('optimizer')
    return out
...
def get_dataloader_new(data_dir, transform, batch_size=128, num_imgs=None, shuffle=False, num_workers=4, collate_fn=None):
    """ Get dataloader for the images in the data_dir.
        If num_imgs > dataset_size, samples will be repeated randomly.
    """
    dataset = ImageFolder(data_dir, transform=transform)
    ...
    if num_imgs is not None:
        if num_imgs > dataset_size:
            print(f"注意: 请求的样本数({num_imgs})超过数据集大小({dataset_size})，将重复采样部分数据")
            # 设置 replace=True 允许重复采样
            indices = np.random.choice(dataset_size, num_imgs, replace=True)
        else:
            indices = np.random.choice(dataset_size, num_imgs, replace=False)

        dataset = Subset(dataset, indices)
        print(f"已采样 {num_imgs} 个样本")
